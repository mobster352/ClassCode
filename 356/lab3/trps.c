...
pthread_cond_t cd1;
pthread_mutex_t lock11;//start2
pthread_cond_t cd11;
pthread_mutex_t lock2;//stop1
pthread_cond_t cd2;
pthread_mutex_t lockt1;//tid1
pthread_cond_t cdt1;
pthread_mutex_t lockt2;//tid2
pthread_cond_t cdt2;
pthread_mutex_t lock22;//stop2
pthread_cond_t cd22;
...
void *child2(void* arg) {
	pthread_mutex_lock(&lockt2);
	ti2 = pthread_self();
	pthread_cond_signal(&cdt2);
	pthread_mutex_unlock(&lockt2);
...
int main(int argc, char* argv[]) {
	...
	int ret9 = pthread_mutex_init(&lock11,NULL);
	int ret10 = pthread_cond_init(&cd11,NULL);
	int ret11 = pthread_mutex_init(&lock22,NULL);
	int ret12 = pthread_cond_init(&cd22,NULL);
	int ret13 = pthread_mutex_init(&lockt1,NULL);
	int ret14 = pthread_cond_init(&cdt2,NULL);
...
printf("or here\n");
	...
	pthread_mutex_destroy(&lock11);
	pthread_mutex_destroy(&lock22);
	pthread_mutex_destroy(&lockt1);
	pthread_mutex_destroy(&lockt2);

	pthread_cond_destroy(&cd1);
	pthread_cond_destroy(&cd2);
	pthread_cond_destroy(&cd3);
	pthread_cond_destroy(&cd4);
	pthread_cond_destroy(&cd11);
	pthread_cond_destroy(&cd22);
	pthread_cond_destroy(&cdt1);
	pthread_cond_destroy(&cdt2);
