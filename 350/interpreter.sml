...
(* checks that formal parameters are unique *)
    fun check_unique (Sexp(h,AtomExp(NIL))) = NIL
    |   check_unique (Sexp(AtomExp(Ident(x)),t)) = if (x=hd(t)) then T
			else check_unique t

    ;

    (* function: check_formal - checks function parameters from matching T or NIL *)
    fun check_formal (Sexp(h,AtomExp(NIL))) = NIL
    |   check_formal (Sexp(AtomExp(Ident(x)),t)) = if (x = "T") then T
			else if (x = "NIL") then T
			else check_formal t
    ;


    (* function: eval_defun - checks defun usage and adds function def to the global d-list *)
    fun eval_defun s a d = if (hd(hd(s)) = AtomExp(Ident("DEFUN"))) then raise EvalError("cannot redefine DEFUN")
			else if (hd(hd(s)) = AtomExp(Ident("COND"))) then raise EvalError("cannot redefine COND")
			else if (hd(hd(s)) = AtomExp(Ident("QUOTE"))) then raise EvalError("cannot redefine QUOTE")
    ...
    fun addpairs [] [] z = z
    |   addpairs xlist ylist z = hd(xlist)::hd(ylist)::(addpairs tl(xlist) tl(ylist) z)
    ;
      
    (* function: eval - top-level s-expression evaluation loop *)
    fun eval (AtomExp(Int(i))) a d = i 
  | eval (AtomExp(Ident(x))) a d = (if (bound (x,a)=T) then (getval (x,a))
  ...
  | eval (AtomExp(x)) a d = 
	...
	(*else*)
  | eval exp a d =
		(if (hd(exp) = AtomExp(Ident("QUOTE"))) then hd(tl(exp))
		else if (hd(exp) = AtomExp(Ident("COND"))) then evcon tl(exp) a d
		else if (hd(exp) = AtomExp(Ident("DEFUN"))) then eval_defun tl(exp) a d
    ...
    and apply (AtomExp(Int(y))) x a d = (if y = "INT" then AtomExp(T)
				else AtomExp(NIL))
    |   apply (AtomExp(Ident(y))) x a d = 
	(*(if f = AtomExp(Ident(y)) then*)
		(if y = "CAR" then hd(hd(x))
		else if y = "CDR" then tl(hd(x))
		else if y = "CONS" then hd(x)::hd(tl(x))
		else if y = "ATOM" then (if (hd(x) = AtomExp(Ident("ATOM"))) then AtomExp(T)
				else AtomExp(NIL))
		...
		else if y = "REMAINDER" then hd(x) mod hd(tl(x))
    ...
    | apply f x a d = eval tl(getval f d) (addpairs hd(getval f d) x a) d
    ;
